        ...
        foreach($acts as $artist){
            ...
            if(!$found){
                ...
                curl_setopt_array($curl, array(
                    ...
                    CURLOPT_URL => "http://gdata.youtube.com/feeds/api/channels?q=%23".str_replace(" ", "", $artistfixed)."&alt=json&start-index=1&max-results=50&v=2",
                ));
                ...
                if(!array_key_exists('entry', $resp['feed'])){
                  echo "<b>Skipped</b> <i>".$artist."</i> for ".$festival." (<a href=\"http://gdata.youtube.com/feeds/api/channels?q=%23".str_replace(" ", "", $artistfixed)."&alt=json&start-index=1&max-results=50&v=2\">Channel not found</a>)<br /><br />";
                  continue;
                }
                ...
                $foundplaylist = false;
                $start = 1;
                foreach($resp['feed']['entry'] as $result){
                  if(strcasecmp($result['title']['$t'], "#".str_replace(" ", "", $artistfixed)) == 0){
                    echo "Using channel: <b>".$result['title']['$t']."</b><br />";
                    $foundchannel = true;
                    do {
                      $curl = curl_init();
                      curl_setopt_array($curl, array(
                          CURLOPT_RETURNTRANSFER => 1,
                          CURLOPT_URL => "http://gdata.youtube.com/feeds/api/users/".$result['yt$channelId']['$t']."/playlists?alt=json&start-index=".$start."&max-results=50&v=2",
                      ));
                      $resptwo = json_decode(curl_exec($curl), true);
                      curl_close($curl); 

                      if($resptwo['feed']['openSearch$totalResults'] < $start || !array_key_exists('entry', $resptwo['feed'])){
                        break;
                      }
                ...
                if($foundplaylist){
                  ...
                  echo "<b>Skipped</b> <i>".$artist."</i> for ".$festival." (<a href=\"http://gdata.youtube.com/feeds/api/users/".$result['yt$channelId']['$t']."/playlists?alt=json&start-index=".$start."&max-results=50&v=2\">Playlist not found</a>)<br /><br />";
                }
            } else {
